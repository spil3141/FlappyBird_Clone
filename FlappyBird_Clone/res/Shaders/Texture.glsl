...
void main()
{
	...
	v_VerticePosition = gl_Position.xy;
}
...
void main()
{
	...
	// Vignette Effect 
	float offset = 0.4f;
	float V_Intensity =  1.0f - distance(v_VerticePosition * offset, vec2(0.0f));
	V_Intensity = clamp(V_Intensity, 0.0f, 1.0f);

	color = vec4(texColor.xyz * V_Intensity,texColor.w);
}
