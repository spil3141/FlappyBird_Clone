...
namespace Tiny {
	...
	public:
		// edited by spil3141
		ServerTemplate(std::string ServerIP, uint16_t port)
		...
		void WaitForClientConnection() // ASYNC - Instruct asio to wait for connection
		{
		...
		void MessageClient(std::shared_ptr<Connection<T>> client, const Packet<T>& msg)
		{
		...
		void MessageAllClients(const Packet<T>& msg, std::shared_ptr<Connection<T>> pIgnoreClient = nullptr)
		{
		...
		void Update(size_t nMaxMessages = -1, bool bWait = false) // Force server to respond to incoming messages
		{
